...
void BSPSubdivision::init() {
  ...
  logger().debug("# nodes need subdivision = {}/{}/{}", nf,
                 processing_n_ids.size(), MC.bsp_nodes.size());
}
...
void BSPSubdivision::subdivideBSPNodes(const Args &args) {
  ...
  while (!processing_n_ids.empty()) {
    ...
    if (args.user_callback) {
      args.user_callback(Step::BSP,
                         double(progress_current) / double(progress_total));
    }
    ++progress_current;
    ...
    for (auto it = nodes[old_n_id].div_faces.begin();
         it != nodes[old_n_id].div_faces.end(); it++) {
      ...
      for (int i = 0; i < nodes[old_n_id].faces.size(); i++) {
        for (int j = 0; j < faces[nodes[old_n_id].faces[i]].vertices.size();
             j++) {
    ...
    for (int i = 0; i < nodes[old_n_id].faces.size(); i++) {
      ...
      for (int j = 0; j < faces[old_f_id].edges.size(); j++) {
        ...
        if (on_vs.size() == 2) {
          if (std::find(on_face.edges.begin(), on_face.edges.end(), old_e_id) ==
              on_face.edges.end())
        ...
        if (pos_vs.size() + on_vs.size() == 2) {
          ...
          if (on_vs.size() > 0 &&
              std::find(on_edge.vertices.begin(), on_edge.vertices.end(),
                        on_vs[0]) == on_edge.vertices.end())
        ...
        int new_v_id = 0;
        Segment_3 seg(vertices[edges[old_e_id].vertices[0]],
                      vertices[edges[old_e_id].vertices[1]]);
        ...
        for (auto it = edges[old_e_id].conn_faces.begin();
             it != edges[old_e_id].conn_faces.end(); it++) {
          if (*it == old_f_id)
            continue;
        ...
        edges[new_e_ids[0]] =
            pos_edge; // if get here, it means that old_edge has been cut into 2
      ...
      if (pos_face.edges.size() == 0 ||
          neg_face.edges.size() == 0) // connected pos/neg
        continue;

      /// from now, the face would definitely be subdivided
      faces.push_back(neg_face); // have to do push_back here!!! Otherwise would
                                 // producing empty faces!!

      /// clean conn_faces for neg_face's edges//fixed
      for (int j = 0; j < neg_face.edges.size(); j++) {
        auto it =
            std::find(edges[neg_face.edges[j]].conn_faces.begin(),
                      edges[neg_face.edges[j]].conn_faces.end(), old_f_id);
      ...
      for (auto it = faces[old_f_id].conn_nodes.begin();
           it != faces[old_f_id].conn_nodes.end(); it++) {
        if (*it == old_n_id)
          continue;
      ...
      for (int j = 0; j < new_f_ids.size(); j++) { // fixed
        for (auto it = faces[new_f_ids[j]].edges.begin();
             it != faces[new_f_ids[j]].edges.end(); it++) /// fixed
    ...
    /// clean conn_nodes for neg_node's faces//fixed //it must be done before
    /// adding on_face!! Otherwise, the on_face would be influenced.
    for (int i = 0; i < neg_node.faces.size(); i++) {
      auto it = std::find(faces[neg_node.faces[i]].conn_nodes.begin(),
                          faces[neg_node.faces[i]].conn_nodes.end(), old_n_id);
    ...
    for (auto it = faces[on_f_id].edges.begin();
         it != faces[on_f_id].edges.end(); it++) /// fixed
...
void BSPSubdivision::calVertexSides(const Plane_3 &pln,
                                    const std::unordered_set<int> &v_ids,
                                    const std::vector<Point_3> &vs,
                                    std::unordered_map<int, int> &v_sides) {
...
int BSPSubdivision::divfaceSide(const Plane_3 &pln,
                                const std::array<int, 3> &p_ids,
