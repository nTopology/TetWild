...
void MeshConformer::matchVertexIndices(
    int x, const std::vector<std::array<int, 2>> &seed_v_list,
...
void MeshConformer::matchDivFaces(const Args &args) {
  ...
  for (int i = 0; i < bsp_faces.size(); i++) {
    for (int j = 0; j < 3; j++)
      seed_v_list.push_back(
          std::array<int, 2>({{bsp_faces[i].vertices[j], i}}));
  }
  std::sort(seed_v_list.begin(), seed_v_list.end(),
  ...
  for (int i = 0; i < m_faces_size; i++) {
    int m_f_id = i;
    std::array<Point_3, 3> tri1 = {{m_vertices[m_faces[i][0]],
    ...
    if (is_matched[i]) {
      ...
      std::set_intersection(f_lists[0].begin(), f_lists[0].end(),
                            f_lists[1].begin(), f_lists[1].end(),
                            std::back_inserter(tmp));
      std::set_intersection(
          tmp.begin(), tmp.end(), f_lists[2].begin(), f_lists[2].end(),
          std::back_inserter(
              tmp1)); // todo: find a better way to cal intersection
      // todo: you can use unordered_set and set a *good* hash function to find
      // identical triangles on bsp tree
      if (tmp1.size() == 1) {
        std::array<int, 3> tmp_bsp = {{bsp_faces[tmp1[0]].vertices[0],
                                       bsp_faces[tmp1[0]].vertices[1],
    ...
    for (auto it = seed_fids.begin(); it != seed_fids.end(); it++)
      for (auto jt = bsp_faces[*it].conn_nodes.begin();
           jt != bsp_faces[*it].conn_nodes.end(); jt++) {
        seed_nids.insert(*jt);
      }

    for (auto it = seed_fids.begin(); it != seed_fids.end(); it++) {
      ////cal intersection type
      std::array<Point_3, 3> tri2 = {
          {bsp_vertices[bsp_faces[*it].vertices[0]],
      ...
      if (int_type == COPLANAR_INT) {
        ...
        for (auto nit = bsp_faces[*it].conn_nodes.begin();
             nit != bsp_faces[*it].conn_nodes.end(); nit++)
      ...
      for (auto it = new_nids.begin(); it != new_nids.end(); it++) {
        for (int j = 0; j < bsp_nodes[*it].faces.size(); j++) {
          ...
          if (fit == seed_fids.end()) {
            ...
            /// check if intersecting (coplanar -> do_intersection / crossing ->
            /// sort 4 interseting points) if intersected -> insert into
            /// new_fids
            /////if coplanar-intersecting -> divface for face
            /////else if crossing-intersecting -> divface for node
            std::array<Point_3, 3> tri2 = {
                {bsp_vertices[bsp_faces[bsp_f_id].vertices[0]],
  ...
  logger().debug("{} faces matched!",
                 std::count(is_matched.begin(), is_matched.end(), true));
}
...
void MeshConformer::getOrientedVertices(int bsp_f_id) {
  ...
  if (vertices.size() != bsp_faces[bsp_f_id].vertices.size()) {
    logger().error("{}, {}", bsp_faces[bsp_f_id].vertices.size(),
                   bsp_faces[bsp_f_id].edges.size());
...
void triangleSideofPlane(const std::array<Point_3, 3> &tri, const Plane_3 &pln,
                         std::vector<Point_3> &pos_vs,
                         std::vector<Point_3> &neg_vs,
                         std::vector<Point_3> &on_vs) {
  for (int i = 0; i < 3; i++) {
...
int MeshConformer::triangleIntersection3d(const std::array<Point_3, 3> &tri1,
                                          const std::array<Point_3, 3> &tri2,
  ...
  for (int i = 0; i < pos_vs1.size(); i++) {
    for (int j = 0; j < neg_vs1.size(); j++) {
      ...
      assert(!(!result));
      if (result) {
        if (std::holds_alternative<Point_3>(*result))
          on_vs1.push_back(std::get<Point_3>(*result));
        else
  ...
  for (int i = 0; i < pos_vs2.size(); i++) {
    for (int j = 0; j < neg_vs2.size(); j++) {
      ...
      if (result) {
        if (std::holds_alternative<Point_3>(*result))
          on_vs2.push_back(std::get<Point_3>(*result));
        else
          throw TetWildError("MeshConformer::triangleIntersection3d");
      }
    }
  ...
  std::sort(
      sorted_vs.begin(), sorted_vs.end(),
      [](const std::pair<Point_3, int> &p1, const std::pair<Point_3, int> &p2) {
...
void MeshConformer::initT(const Vector_3 &nv) {
  std::vector<Vector_3> vs{Vector_3(1, 0, 0), Vector_3(0, 1, 0),
                           Vector_3(0, 0, 1)};
