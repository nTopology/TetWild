...
void MshLoader::parse_elements(std::ifstream& fin) {
    ...
    std::vector<int> tet_element_quadratic_idx;
    std::vector<int> tet_elements_quadratic;
    ...
    auto get_element_idx_storage = [&](int elem_type) -> std::vector<int>* {
        switch (elem_type) {
            ...
            case 11:
                return &tet_element_quadratic_idx;
            default:
                throw IOError("Unsupported element type encountered");
        };
    };
    ...
    if (!tet_elements.empty()) {
        ...
        m_element_type = 3;
    } else if (!tet_elements_quadratic.empty()) {
        copy_to_array(tet_elements_quadratic, 10);
        m_element_type = 11;
    } else {
...
int MshLoader::num_nodes_per_elem_type(int elem_type) {
    ...
    switch (elem_type) {
        ...
        case 11:
            nodes_per_element = 10; // quadratic tet
            break;
        default:
